...
Final local verification: all 39 unit tests across 10 files passed, plus lint and strict typecheck. Both production paths (`/` and `/Kanban/`) passed all 28 browser tests using Chrome on Windows, about 1.3 minutes per suite. Formatting and Git whitespace checks pass. The existing main-bundle size advisory remains.

Publication follow-up, 17 September: changes are pushed in PR #9. The first GitHub run passed 27 of 28 browser tests; the keyboard-drag test raced asynchronous movement. Commit `e21bf4b` waits for visible movement before dropping and passed five local repetitions. Current hosted results are available on the PR. Pages source is now GitHub Actions, resolving the disabled-source configuration; deployment awaits merge. Manual speech-output acceptance and the TypeScript 7 deferral remain outstanding.
